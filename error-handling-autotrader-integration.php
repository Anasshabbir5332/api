...
class AutoTrader_Integration {
            ...
            jQuery(document).ready(function($) {
                $('#sync-autotrader-btn').on('click', function() {
                    ...
                    $.ajax({
                        ...
                        // error: function() {
                        //     $status.html('<p>Error occurred during sync (table error). Please try again.</p>');
                        //     $button.prop('disabled', false);
                        // }
                        ...
                        // Show detailed error (fallback to generic if empty)
                        const errorMsg = jqXHR.responseText || textStatus || "Unknown error occurred";
                        $status.html(`<p>Sync failed: ${errorMsg}</p>`);
                        $button.prop('disabled', false);
                        }
                    });
                });
    ...
    public function sync_autotrader_listings($advertiser_id = '10012495', $max_pages = 0, $sync_type = 'automatic') {
        set_time_limit(0); // Attempt to prevent script timeout during long syncs
        ...
        try {
            ...
            if (empty($stock_data) || !is_array($stock_data)) {
                throw new Exception('No stock data received from API or invalid format');
            }
            ...
            return "Sync completed: $created listings created, $updated listings updated, $deleted listings deleted, $skipped listings skipped.";
            ...
            $this->log('Error during sync: ' . $error_message);
            ...
            return 'Error during sync: ' . $error_message;
        }
    }
    ...
    private function send_sync_report_email($log_data) {
        ...
        $subject = "[{$site_name}] AutoTrader Sync Report Dev Motorman - {$sync_time}";
        
        $body = "<h2>AutoTrader Sync Report Dev Motorman </h2>";
