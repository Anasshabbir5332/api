...
class TF_Listing {
    ...
    private $api_secret = '3345354';
    private $advertiser_id = '697';
    private $auth_token = '';
    ...
    /**
     * Authenticate with AutoTrader API
     */
    private function authenticate_api() {
        ...
        $response = wp_remote_post($this->auth_endpoint, [
            'headers' => [
                'Content-Type' => 'application/json',
                'X-Api-Key' => $this->api_key,
                'X-Api-Secret' => $this->api_secret
            ],
            'body' => json_encode(['advertiserId' => $this->advertiser_id]),
            'timeout' => 10
        ]);
        ...
        if (!is_wp_error($response) && 200 === wp_remote_retrieve_response_code($response)) {
            ...
            $this->token_expiry = time() + 3500; // 1 hour - 100s buffer
            return !empty($this->auth_token);
        }
        
        error_log('AutoTrader API authentication failed');
        return false;
    }
     ...
     */
    private function get_api_data($endpoint, $params = []) {
        if (!$this->authenticate_api()) {
            return false;
        }
        
        $transient_key = 'tfcl_api_' . md5($endpoint . serialize($params));
        $data = get_transient($transient_key);
        
        if (false === $data) {
            $url = add_query_arg($params, $this->stock_endpoint . $endpoint);
    ...
    /**
     * Transform API vehicle data to our format
     */
    private function transform_vehicle_data($vehicle) {
        return [
            ...
            'model' => $vehicle['model'] ?? '',
            'year' => $vehicle['year'] ?? '',
            'price' => isset($vehicle['price']['amount']) ? number_format($vehicle['price']['amount']) : 'N/A',
            'mileage' => isset($vehicle['mileage']['value']) ? number_format($vehicle['mileage']['value']) . ' miles' : '',
            ...
            'location' => [
                ...
                'address' => $vehicle['location']['address'] ?? 'Location not specified'
            ],
            'specs' => $vehicle['specification'] ?? [],
            'description' => $vehicle['description'] ?? '',
            'url' => '#listing-' . ($vehicle['vehicleId'] ?? '')
        ];
    }
...
//start tfcl_listing_map_shortcode *******************
    /**
     * Listing Map Shortcode - API Version
     */
    public function tfcl_listing_map_shortcode($atts) {
        wp_enqueue_script('google-maps');
        wp_enqueue_script('tfcl-listing-map');
        wp_enqueue_style('tfcl-listing-map');
        ...
        $api_params = [
            ...
            'pageSize' => (int)$atts['limit']
        ];
        
        // Add filters
        if ($atts['make']) $api_params['make'] = sanitize_text_field($atts['make']);
        if ($atts['model']) $api_params['model'] = sanitize_text_field($atts['model']);
        if ($atts['price_min']) $api_params['priceFrom'] = (int)$atts['price_min'];
        if ($atts['price_max']) $api_params['priceTo'] = (int)$atts['price_max'];
        if ($atts['radius']) $api_params['radius'] = (int)$atts['radius'];
        
        // Get listings
        $api_response = $this->get_api_data('/search', $api_params);
        $listings = [];
        
        if (!empty($api_response['vehicles'])) {
            foreach ($api_response['vehicles'] as $vehicle) {
                $listings[] = $this->transform_vehicle_data($vehicle);
            }
        }
        
        // Prepare template
        ob_start();
        include plugin_dir_path(__FILE__) . 'listing-map.php';
        return ob_get_clean();
    }
//end  tfcl_listing_map_shortcode **************
