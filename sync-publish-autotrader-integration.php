...
class AutoTrader_Integration {
    ...
    public function add_custom_cron_schedules($schedules) {
        $schedules["every_8_hours"] = array(
            "interval" => 8 * HOUR_IN_SECONDS, // 8 hours
    ...
    public function sync_frequency_callback() {
        ...
        echo '<option value="every_8_hours" ' . selected("every_8_hours", $frequency, false) . '>Every 8 Hours</option>'; // Add new option
        echo '</select>';
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                $('#sync-autotrader-btn').on('click', function() {
                    ...
                    $status.html('<div class="notice notice-info"><p><span class="spinner is-active" style="float:left; margin-right:10px;"></span> Starting sync process...</p></div>');
                    
                    // Create progress bar container
                    $status.append('<div class="sync-progress-container" style="margin-top: 15px; display: none;">' +
                        '<div class="sync-progress-bar" style="height: 20px; background-color: #0073aa; width: 0%; transition: width 0.5s;"></div>' +
                        '<div class="sync-progress-text" style="margin-top: 5px; font-weight: bold;">0%</div>' +
                    '</div>');
                    ...
                    function processBatch() {
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                var percentComplete = Math.round((processed / total) * 100);
                                
                                // Update progress bar and text
                                $('.sync-progress-bar').css('width', percentComplete + '%');
                                $('.sync-progress-text').text(percentComplete + '% (' + processed + ' of ' + total + ' listings processed)');
                                
                                // Update status message
                                $status.find('.notice p').html('<span class="spinner is-active" style="float:left; margin-right:10px;"></span> ' + response.data.message);
                                
                                // If more batches are pending, process the next batch automatically
                                if (response.data.more_batches_pending === true) {
                                    // Short delay before next batch to prevent overwhelming the server
                                    ...
                                    // Refresh the recent history section after a short delay
                                    setTimeout(function() {
                                        location.reload();
                                    }, 3000);
                                }
                                ...
                                // Error handling
                                $status.html('<div class="notice notice-error"><p>❌ Error: ' + (response.data || 'Unknown error occurred') + '</p></div>');
                                $button.prop('disabled', false);
                            }
                        },
                        error: function(xhr, status, error) {
                            $status.html('<div class="notice notice-error"><p>❌ Connection error occurred. Please try again.</p></div>');
                            $button.prop('disabled', false);
                        }
                    });
                }
    ...
    public function sync_autotrader_listings($sync_type = "manual") {
        ...
        try {
            ...
            $deleted_count = $this->delete_old_listings(
                $batch_state["existing_listings"],
                $batch_state["processed_ids"],
                $batch_state["api_data"] // Make sure this contains the vehicle data array
            );
                ...
                return array(
                    ...
                    "processed_items" => isset($batch_state["processed_items"]) ? $batch_state["processed_items"] : 0,
                    "total_items" => isset($batch_state["total_items"]) ? $batch_state["total_items"] : 0
                );
            } else {
                // --- More Batches Needed ---
                ...
                // Return progress information for auto-batching
            ...
            $error_log_data = array(
                'sync_time' => current_time('mysql'),
                'sync_type' => isset($batch_state["sync_type"]) ? $batch_state["sync_type"] : $sync_type,
                'advertiser_id' => isset($batch_state["advertiser_id"]) ? $batch_state["advertiser_id"] : $advertiser_id,
                'created_count' => isset($batch_state["total_created"]) ? $batch_state["total_created"] : 0,
                'updated_count' => isset($batch_state["total_updated"]) ? $batch_state["total_updated"] : 0,
                'deleted_count' => 0, // Deletion happens only at the end
                'skipped_count' => isset($batch_state["total_skipped"]) ? $batch_state["total_skipped"] : 0,
                'status' => "failed",
                'duration' => microtime(true) - $overall_start_time,
                'error_message' => $error_msg,
                'details' => json_encode(array(
                    "skipped_listings" => isset($batch_state["total_skipped_listings"]) ? $batch_state["total_skipped_listings"] : array()
                ))
            );
            $this->save_sync_log($error_log_data);
            
            // Send email report if enabled
            if (get_option("autotrader_email_reports", 0)) {
                $this->send_sync_report_email($error_log_data);
            }
     ...
     * Delete old listings
     */
private function delete_old_listings($existing_listings, $processed_ids, $api_response = null) {
    $deleted_count = 0;
    $to_delete_post_ids = [];

    // Create a quick lookup for API data by stock_number
    $api_data_by_stock_number = [];
    if (is_array($api_response)) {
        foreach ($api_response as $car) {
            if (!empty($car['stock_number'])) {
                $api_data_by_stock_number[$car['stock_number']] = $car;
            }
        }
    }

    // Identify listings to delete that are no longer in the API at all
    $removed_from_api = array_diff($existing_listings, $processed_ids);
    foreach ($removed_from_api as $post_id) {
        $to_delete_post_ids[$post_id] = true;
    }

    // Identify listings to delete that are NOT_PUBLISHED in the API
    foreach ($existing_listings as $post_id) {
        // Skip if already marked for deletion
        if (isset($to_delete_post_ids[$post_id])) {
            continue;
        }

        $stock_number = get_post_meta($post_id, 'stock_number', true);
        
        if ($stock_number && isset($api_data_by_stock_number[$stock_number])) {
            $car_data = $api_data_by_stock_number[$stock_number];
            $status = $car_data["adverts"]["retailAdverts"]["advertiserAdvert"]["status"] ?? null;
    ...
    foreach ($to_delete_post_ids as $post_id => $value) {
